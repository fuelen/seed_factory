defmodule SeedFactoryTest do
    ...
    {context, diff} =
      with_diff(context, fn ->
        ...
        |> rebind([office: :office1, user: :user1, profile: :profile1], fn context ->
          ...
          |> exec(:create_pending_user, name: "John Doe")
        end)
  ...
  test "exec command with map option - applies map function to entity", context do
    context = exec(context, :create_pending_user)
  ...
  test "exec command with map option - doesn't apply map function to explicitly passed parameter",
       context do
    context = exec(context, :create_pending_user, office_id: "MY_OFFICE_ID")
  ...
  describe "traits" do
    ...
    test "automatic detection of traits from params", context do
      context
      |> exec(:create_pending_user, role: :admin)
    ...
    test "specify traits which conflict with requirements of other entities", context do
      ...
      assert_raise ArgumentError,
                   """
                   Cannot merge arguments generated by traits for command :create_pending_user.
                     Path: [:role]
                     Value 1: :normal
                     Value 2: :admin
                   """,
  ...
  test "pre_exec", context do
    {_context, diff} =
      with_diff(context, fn ->
        pre_exec(context, :create_pending_user)
      end)
    ...
    {_context, diff} =
      with_diff(context, fn ->
        pre_exec(context, :create_pending_user, office_id: :fake_office_id)
      end)
